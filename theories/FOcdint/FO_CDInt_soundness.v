...
Proof.
intros A AX. inversion AX ; subst ; intros w alpha ass u rho ; cbn ; intros ; auto.
...
- apply ksat_comp. unfold funcomp. eapply (ksat_ext alpha v). 2: eapply (H0 (eval rho _)).
  intros. unfold scons. destruct x ; auto.
- apply ksat_comp in H0. eexists (eval rho t). eapply (ksat_ext _ v). 2: eapply H0.
  unfold funcomp. intros. unfold scons. destruct x ; auto.
- destruct (LEM ((ass ⊩( alpha, w) v) rho B)) ; auto. (* Use of LEM *)
  left. intros. destruct (H0 j) ; auto. exfalso. apply H1. apply ksat_comp in H2.
  eapply (ksat_ext _ v). 2: exact H2. intros. unfold funcomp. unfold scons.
  cbn ; auto.
Qed.

Lemma Soundness : forall Γ A, FOCDIH_prv Γ A -> loc_conseq Γ A.
Proof.
intros Γ A Hprv. induction Hprv; subst; cbn; intros D M u ass rho HX.
(* Id *)
- apply HX ; auto.
(* Ax *)
- apply Ax_valid ; auto.
(* MP *)
- unfold kvalid_ctx in IHHprv1,IHHprv2. pose (IHHprv1 _ _ _ _ _ HX). pose (IHHprv2 _ _ _ _ _ HX).
...
- intros ass0 Ha0 Ha1. unfold loc_conseq in IHHprv. destruct Ha1.
  assert (J2: (forall psi : form, (fun x : form => exists B : form, x = B[↑] /\ In form Γ B) psi -> (u ⊩( M, D) ass) (x .: rho) psi)). intros.
  destruct H0. destruct H0. subst. apply HX in H1. apply ksat_comp.
  simpl. unfold funcomp. simpl. auto. pose (IHHprv _ _ u _ (x .: rho) J2).
