...
Theorem FOBIH_subst : forall Γ A f, FOBIH_prv Γ A ->
    FOBIH_prv (fun x : form => exists B : form, x = B[f] /\ In form Γ B) (A[f]).
Proof.
...
(* DN *)
- pose (IHD f). apply DN. apply (FOBIH_monot _ _ f0).
  intros B HB ; destruct HB as (C & H0 & H1) ; subst ; inversion H1.
(* Gen *)
- pose (IHD (up f)). cbn in *. apply Gen.
  apply (FOBIH_monot _ _ f0). intros C HC ; destruct HC as (B & HB1 & HB2) ; subst.
...
- pose (IHD (up f)). cbn in *. eapply EC. rewrite up_form in f0.
  apply (FOBIH_monot _ _ f0). intros C HC ; destruct HC as (E & HE1 & HE2) ; subst.
...
- destruct IHD0_1 as (Fin0 & HF00 & HF01 & (l0 & Hl0)) ;
  ...
  intros C HC ; inversion HC ; auto.
  eapply MP. apply (FOBIH_monot _ _ HF01).
  intros C HC ; left ; auto.
  apply (FOBIH_monot _ _ HF11). intros C HC ; right ; auto.
...
- destruct IHD0 as (Fin & HF0 & HF1 & (l & Hl)).
  ...
  intros C HC ; auto.
  apply Gen. apply (FOBIH_monot _ _ HF1) ; auto.
...
- destruct IHD0 as (Fin & HF0 & HF1 & (l & Hl)).
  ...
  exists (fun y => List.In y l0). repeat split.
  intros C HC ; auto.
  apply EC. apply (FOBIH_monot _ _ HF1) ; auto.
  intros C HC. unfold In. apply Hl in HC. apply in_map_iff in HC.
  destruct HC. exists x ; auto ; split ; firstorder.
  exists l0 ; auto.
Qed.
